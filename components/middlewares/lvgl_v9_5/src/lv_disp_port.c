...
static lv_display_t *s_disp_drv;
...
void lv_port_disp_init(plat_lcd_dev_t *lcd_dev)
{
    ...
    s_disp_drv = lv_display_create(lcd_dev->width, lcd_dev->height);
    assert(s_disp_drv);
    ...
    lv_display_set_flush_cb(s_disp_drv, _lv_port_flush_cb);
...
static void _lv_port_flush_cb(lv_display_t *drv, const lv_area_t *area, uint8_t *color_map)
{
    plat_lcd_dev_flush(s_lcd_dev, area->x1, area->y1, area->x2 + 1, area->y2 + 1, color_map);
}
