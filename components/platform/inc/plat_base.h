...
#define PLAT_DEV_LOCK_FROM_ISR(ptr)   (xSemaphoreTakeFromISR(PLAT_GET_BASE(ptr)->lock)), PLAT_GET_BASE(ptr)->state = PLAT_STATE_BUSY)
#define PLAT_DEV_UNLOCK_FROM_ISR(ptr) (xSemaphoreGiveFromISR(PLAT_GET_BASE(ptr)->lock)), PLAT_GET_BASE(ptr)->state = PLAT_STATE_READY)
