...
void plat_key_dev_register(plat_key_dev_t *key, const char *name,
                            const plat_dev_ops_t *base_ops, const plat_key_ops_t *ops, void *priv)
{
...
{
    PLAT_DEV_CHECK(key);
    return key->ops->read_raw(key, state);
}