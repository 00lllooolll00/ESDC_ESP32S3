...
void init_style_func_card_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_shadow_width(style, 10);
    lv_style_set_bg_color(style, lv_color_hex(0x6b7682));
    lv_style_set_layout(style, LV_LAYOUT_FLEX);
    lv_style_set_flex_main_place(style, LV_FLEX_ALIGN_CENTER);
    lv_style_set_flex_cross_place(style, LV_FLEX_ALIGN_CENTER);
    lv_style_set_flex_track_place(style, LV_FLEX_ALIGN_CENTER);
...
lv_style_t *get_style_func_card_MAIN_DEFAULT() {
    ...
    if (!style) {
        style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_func_card_MAIN_DEFAULT(style);
    }
    return style;
};
...
void add_style_func_card(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_func_card_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_func_card(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_func_card_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};
